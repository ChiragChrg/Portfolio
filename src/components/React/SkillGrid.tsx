...
const SkillGrid = ({ stack }: Prop) => {
    ...
    return (
        <div className="Fade_Stagger grid grid-cols-3 lg:grid-cols-5 gap-4">
            {stack.length <= maxIcons ?
                stack.map((obj, i) => {
                    // While stack.length <= maxIcons, show all icons
                    return <a key={i} href={obj.link} title={obj.name} className="flex_center flex-col gap-2 min-w-24 aspect-square rounded-lg bg-skillGradient" target="_blank" rel="noreferrer">
